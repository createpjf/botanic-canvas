...
| Agent 工具流式时间线 | `src/domain/agentTimeline.ts`、`src/domain/agentChatStream.ts` | `server/agentToolRuntime.mjs`、`/api/agent-chat/stream`、`/api/agent-intent/stream`、`/api/agent-plans/stream`、`AgentWorkspace.tsx`、`AgentToolOrb.tsx` | 工具步仅在 `registry.execute` 前后 emit；禁止客户端 rAF 假进度；确认后 Run/分支投影进同款时间线；`agentTimelineOrbState` 只映射动画态 |
