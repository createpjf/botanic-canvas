...
| 普通生成任务 | `src/lib/generationApi.ts` | `server/generationService.mjs`、`generationProcessor.mjs`、`generationProvider.mjs`、`generationOutputSize.mjs` | `server/generation*.test.mjs`、`src/domain/generationOutputSize.test.ts`；同一次重试复用幂等键；gpt-image-2 可自定义像素 |
...
| 局部重绘（选区→蒙版） | `src/domain/regionMask.ts` | `server/regionMaskPng.mjs`、`generationProvider.mjs`、`RegionMaskEditor.tsx`、`agent.ts`（`region_edit`） | regionMask/regionMaskPng/provider 测试；选区是归一化纯数据可进计划与 recipe，位图蒙版由 Worker 按基准图真实像素生成；蒙版能力由模型目录 `supportsMask` 声明 |
| MCoT 创意分解 | `src/domain/agentCreativeComposition.ts` | `server/botanicAgentTurn.mjs`（`decompose_creative_brief`）、`agentVariations.ts`（条目分支）、`botanicAgentExecution.mjs` | 分解/执行测试；成套请求先出结构化方案卡，「生成第 N 项」逐项直落，「执行方案」按条目展开为异构分支 Run（图片/视频混排） |
