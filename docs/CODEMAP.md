...
| 界面动效 | `src/components/gsapMotion.ts` | `ProductLanding.tsx`、`AgentComposer.tsx`、`AgentWorkspace.tsx`、`AgentConversationMessage.tsx`、`GenerationDotsField.tsx`、`generationDotsField.ts`、`AgentThinkingOrb.tsx` | `src/components/gsapMotion.test.ts`、`GenerationDotsField.test.ts`、`AgentThinkingOrb.test.ts`；只动 transform/opacity；工具面板用 Flip（simple，不插值宽高）；结果节点生成中铺满 snake 点阵场，循环相位、不接线 `generationProgress`、不伪造业务百分比；不伪造工具进度；reduced-motion 停在静帧 |
