...
| 界面动效 | `src/components/gsapMotion.ts` | `ProductLanding.tsx`、`AgentComposer.tsx`、`AgentWorkspace.tsx`、`AgentConversationMessage.tsx`、`LiquidProgressBar.tsx`、`liquidProgressShader.ts`、`generationLiquidTravel.ts`、`AgentThinkingOrb.tsx` | `src/components/gsapMotion.test.ts`、`LiquidProgressBar.test.ts`、`generationLiquidTravel.test.ts`、`AgentThinkingOrb.test.ts`；只动 transform/opacity；工具面板用 Flip（simple，不插值宽高）；结果节点 liquid 单次从左到右，跟任务阶段和 `submittedAt` 对齐，不循环、不接线 `generationProgress`、不伪造业务百分比；不伪造工具进度；reduced-motion 时长为 0 |
