...
| 局部重绘（选区→蒙版） | `src/domain/regionMask.ts` | `server/regionMaskPng.mjs`、`generationProvider.mjs`、`generationComposition.mjs`、`RegionMaskEditor.tsx`、`agent.ts`（`region_edit`） | regionMask/regionMaskPng/provider/composition 测试；选区是归一化纯数据可进计划与 recipe，位图蒙版由 Worker 按基准图真实像素生成；蒙版能力由模型目录 `supportsMask` 声明；本轮 @ 或原配方标识图可进入选区合成 |
