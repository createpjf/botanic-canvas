...
- **MCP 工具走服务端版本化能力目录。** 每项声明 `server`、`tool`、版本、输入/输出 Schema 与 HTTPS 地址；确认提案固定 capability hash，执行前发现配置漂移会拒绝。浏览器收不到地址或凭据，外部调用仍需你逐次确认。
- **联网检索是可选的，且不由浏览器发起。** 未配置检索密钥时没有 `web_search`。
- **上传素材逐项校验**：单文件 8 MB 上限、MIME 与 PNG / JPEG / WebP 文件签名。参考图另有 16.78 MP 像素接收预算（4096×4096，覆盖 Nano Banana 4K 方图），以及 80 MP 解码上限用于防解压炸弹。GPT Image 2 自定义像素窗仍是 8.29 MP。
