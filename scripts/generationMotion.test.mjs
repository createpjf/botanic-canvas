...
test('生成中结果节点铺满 flow 点阵并尊重减少动效设置', () => {
  ...
  assert.match(styles, /\.result-node--generating \+ \.result-node__task-copy\s*{[^}]*justify-content:\s*space-between/s)
  ...
  assert.match(styles, /\.result-node__task-state--generating \.result-node__task-action\s*{[^}]*min-height:\s*32px/s)
  assert.match(styles, /\.result-node__task-state--generating \.result-node__task-action\s*{[^}]*font-size:\s*11px/s)
  assert.match(resultNode, /isGenerating \? \(\s*<div className="result-node__task-copy">/s)
  assert.match(resultNode, /elapsedTaskLabel\(elapsedSeconds, locale\)/)
  assert.doesNotMatch(resultNode, /isSlowTask/)
  assert.doesNotMatch(resultNode, /taskFeedback\.detail/)
