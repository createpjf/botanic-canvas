...
test('生成中结果节点与任务卡提供真实动效并尊重减少动效设置', () => {
  ...
  assert.match(styles, /\.liquid-progress-bar\s*{/)
  assert.match(styles, /@keyframes\s+result-task-sheen/)
  assert.match(resultNode, /<LiquidProgressBar\b/)
  assert.match(liquidRuntime, /prefersLiquidReducedMotion|prefers-reduced-motion:\s*reduce/)
  ...
  assert.match(
    styles,
    /@media\s*\(prefers-reduced-motion:\s*reduce\)\s*{[^}]*\.result-node__task-state--generating::before,[^}]*animation:\s*none/s,
  )
  assert.match(
    styles,
