...
test('生成中结果节点铺满 flow 点阵并尊重减少动效设置', () => {
  ...
  assert.match(styles, /\.result-node--generating \+ \.result-node__task-copy\s*{[^}]*margin:\s*4px 0 0/s)
  assert.match(styles, /\.result-node--generating \+ \.result-node__task-copy strong,\s*\.result-node--generating \+ \.result-node__task-copy small\s*{[^}]*white-space:\s*nowrap/s)
  assert.match(styles, /\.result-node__task-state--generating \.result-node__task-action\s*{[^}]*position:\s*absolute/s)
  assert.match(styles, /\.result-node__task-state--generating \.result-node__task-action\s*{[^}]*right:\s*8px/s)
  assert.match(styles, /\.result-node__task-state--generating \.result-node__task-action\s*{[^}]*bottom:\s*8px/s)
  assert.match(resultNode, /isGenerating \? \(\s*<div className="result-node__task-copy">/s)
