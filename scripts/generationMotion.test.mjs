...
test('生成中结果节点与任务卡提供真实动效并尊重减少动效设置', () => {
  ...
  assert.match(resultNode, /taskStatus=\{result.taskStatus\}/)
  assert.match(resultNode, /submittedAt=\{result.submittedAt\}/)
  assert.doesNotMatch(resultNode, /generationProgress/)
  assert.match(liquidRuntime, /generationLiquidTravel/)
  assert.doesNotMatch(liquidRuntime, /liquidIndeterminateTravel/)
