...
test('Postgres / Supabase 项目列表不得读取整份 document JSONB', () => {
  const postgres = listProjectsSource('../server/store/postgresProductStore.mjs')
  const supabase = listProjectsSource('../server/store/supabaseProductStore.mjs')
...
test('Postgres Agent 消息读取按会话截断，避免单会话膨胀顶满语句超时', () => {
  const source = readFileSync(new URL('../server/store/postgresProductStore.mjs', import.meta.url), 'utf8')
