...
function projectDocument() {
  return {
    ...
    generationJobs: [{ id: 'job-busy', status: 'running' }],
  }
}
...
test('画布编辑执行器：改文字回增量 patch，调参校验模型目录，删除回节点清单', async () => {
  ...
  const executors = createCanvasAgentEditExecutors({
    ...
    models: [{ id: 'gpt-image-2' }, { id: 'minimax-image-01' }],
    userId: 'user-1',
    projectId: 'project-1',
  })
...
test('活跃任务绑定的节点不可删除，历史与任务恢复语义不受编辑影响', () => {
  assert.throws(
    () => applyBotanicAgentCanvasNodeDeletion(projectDocument(), { nodeIds: ['result-busy'] }),
    (caught) => caught.code === 'CANVAS_NODE_BUSY',
  )
  // 删除生成节点会连带清理它的系统连线，但不触碰 generationJobs 记录（历史权威）。
  const removed = applyBotanicAgentCanvasNodeDeletion(projectDocument(), { nodeIds: ['generate-1'] })
