...
function fail(error, fallback = 'Supabase 数据操作失败。') {
  if (error) throw productError(error.message || fallback, error.code)
}
...
export function createSupabaseProductStore({ url, secretKey, bootstrapEmail, inviteRedirectTo }) {
  ...
  return {
    ...
    async readGlobalAssetLibrary(userId, id) {
      const { count, error: accessError } = await supabase.from('project_members').select('*', { count: 'exact', head: true }).eq('user_id', userId)
      fail(accessError)
    ...
    async readGenerationJob(userId, jobId) {
      const { data, error } = await supabase.from('generation_jobs').select('payload').eq('id', jobId).eq('owner_id', userId).maybeSingle()
      fail(error)
    ...
    async recoverGenerationJobs() {
      const { data, error } = await supabase.from('generation_jobs').select('payload').eq('status', 'queued').order('updated_at', { ascending: true })
      fail(error)
    ...
    async readMediaObject(userId, mediaId) {
      ...
      return { id: data.id, storageKey: data.storage_key, contentType: data.content_type, byteSize: Number(data.byte_size) }
    },
