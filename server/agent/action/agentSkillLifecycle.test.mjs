...
test('Skill 从草稿经检查发布并以历史版本恢复为新草稿', () => {
  ...
  const published = publishReviewedAgentSkill(review, { actorId: 'u-1', expected: { version: 1, contentHash: review.contentHash }, riskOf, skillCatalog: [], now: 200 })
  assert.equal(published.lifecycle, 'published'); assert.equal(published.updatedAt, 201)
  ...
  assert.equal(deprecatePublishedAgentSkill(republished, { actorId: 'u-1', expected: { version: 3, contentHash: republished.contentHash }, now: 700 }).lifecycle, 'deprecated')
