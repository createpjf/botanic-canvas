...
import {
  botanicAgentSkillRisk,
  resolveBotanicAgentAvailableSkills,
  resolveBotanicAgentMountedSkills,
} from './botanicAgentSkillCatalog.mjs'

// Skill 目录/挂载/风险语义已拆至 botanicAgentSkillCatalog.mjs;这里保留 re-export 兼容既有消费者。
export {
  botanicAgentSkillRisk,
...
export function createBotanicAgentPlanningToolRegistry({ input, finalizePlan, finalizeClarification, onProposeAction, webResearch, subagentRunner, operations }) {
  ...
  const tools = [
    ...
    }] : []),
    {
      ...
      label: '提议画布修改',
      description: '提议需要用户确认的画布修改；优先用 action_set 合成原子行动，用 layout_nodes 声明布局意图，并用 Frame/Stage 组织生产泳道；成员始终保留画布绝对坐标。结果图片、任务绑定与系统连线永不可改。',
      risk: 'read',
      parameters: {
        type: 'object', additionalProperties: false,
        properties: {
