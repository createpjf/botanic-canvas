...
test('PostgreSQL 与 Supabase Adapter 均实现五方法，迁移提供对应原子 RPC', () => {
  const postgresAdapter = readFileSync(new URL('../store/postgresProductStore.mjs', import.meta.url), 'utf8')
  const supabaseAdapter = readFileSync(new URL('../store/supabaseProductStore.mjs', import.meta.url), 'utf8')
...
test('三个 Adapter 的普通 putAgentRun 都在锁内按分支合并，Supabase RPC 不再 whole-row LWW', () => {
  const localAdapter = readFileSync(new URL('../store/productStore.mjs', import.meta.url), 'utf8')
  const postgresAdapter = readFileSync(new URL('../store/postgresProductStore.mjs', import.meta.url), 'utf8')
  const migration = readFileSync(new URL('../../supabase/migrations/20260827170000_generation_job_execution_fence.sql', import.meta.url), 'utf8')
