...
test('动态 HTTP 路由目录区分项目、Agent、任务与用户资源', () => {
  ...
  assert.deepEqual(matchBotanicHttpRoutes('/api/projects/project-1/canvas-sync').projectCanvasSync?.slice(1), ['project-1'])
  ...
  assert.deepEqual(matchBotanicHttpRoutes('/api/projects/project-1/agent-sessions/session-1/context-compactions').agentSessionContextCompactions?.slice(1), ['project-1', 'session-1'])
