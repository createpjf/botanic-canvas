...
test('最新短消息只 mood，大回复限次 wow 且 28px 不出字', async ({ page }) => {
  ...
  await injectAssistantMessage(page, { id: 'bob-mid-reply', content: '字'.repeat(80) })
  const midRole = agent.locator('[data-agent-message-id="bob-mid-reply"] .agent-message__role')
  await expect(midRole).toHaveAttribute('data-bob-size', 'compact')
  await expect(midRole).toHaveAttribute('data-bob-says', 'none')
  await expect(agent.locator('[data-agent-message-id="bob-mid-reply"] .agent-message')).not.toHaveClass(/is-bob-large/)
  ...
  await expect(largeRole).toHaveAttribute('data-bob-size', 'large')
