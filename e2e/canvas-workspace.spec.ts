...
test('Agent 生成卡片默认收起已完成步骤与提示词差异，主内容保持清晰', async ({ page }) => {
  ...
  const promptReview = page.locator('details.agent-prompt-review').first()
  await expect(promptReview).not.toHaveAttribute('open', '')
  await promptReview.locator(':scope > summary').click()
  ...
  await expect(page.getByRole('textbox', { name: '润色后提示词' })).toHaveValue('保持人物、五官、服装与姿态不变，仅将背景替换为晴朗海边。')
...
test('已提交生成卡默认折叠，规格可见且展开后可查看提示词与恢复说明', async ({ page }) => {
  await stubReadOnlyRuntime(page)
  await page.goto('/#/projects')
  await page.getByRole('button', { name: '新建项目' }).click()
  await page.getByRole('button', { name: '描述目标', exact: true }).click()
  ...
  await expect(page.getByRole('heading', { name: '已提交', exact: true })).toBeVisible()
  ...
  await expect(page.locator('.ai-plan__description')).toContainText('3:4')
  await expect(page.getByRole('button', { name: '查看提交详情' })).toHaveAttribute('aria-expanded', 'false')
  await page.getByRole('button', { name: '查看提交详情' }).click()
  await page.locator('details.agent-prompt-review > summary').click()
...
test('最新短消息只 mood，大回复限次 wow 且 28px 不出字', async ({ page }) => {
  ...
  const largeSlotBox = await largeRole.boundingBox()
  const largeVisualBox = await largeRole.locator('svg').boundingBox()
