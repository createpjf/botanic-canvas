...
export function timelineRawGroup(blocks: TimelineBlock[]) {
  return blocks.find((block): block is TimelineRawGroupBlock => block.type === 'raw_group')
}

export function semanticBlocks(blocks: TimelineBlock[]) {
...
export function isSubmitStep(block: TimelineBlock): block is TimelineStepBlock {
  return block.type === 'step' && block.kind === 'write' && /提交/u.test(block.title)
}

export function isGenerateStep(block: TimelineBlock): block is TimelineStepBlock {
  return block.type === 'step' && block.kind === 'write' && (/^生成/u.test(block.title) || /等待生成结果/u.test(block.title))
}

export function isPrepareStep(block: TimelineBlock): block is TimelineStepBlock {
...
// 工具手风琴/图标/MCP品牌展示已拆至 agentToolAccordion.ts(消费者直接从该模块导入,避免运行时依赖环)。
