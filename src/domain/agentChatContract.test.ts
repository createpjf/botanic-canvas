...
test('通用 Agent 对话只发送稳定当前消息，不上传客户端历史', () => {
  const request = buildBotanicAgentChatRequest({
    projectId: 'project-chat',
    sessionId: 'session-chat',
    inputMessage: { id: 'message-current', content: '查当前项目资料' },
    locale: 'en',
  ...
  assert.deepEqual(request.inputMessage, { id: 'message-current', content: '查当前项目资料' })
