    ...
    productionWorkflowRuns: Array.isArray(legacy.productionWorkflowRuns)
      ? legacy.productionWorkflowRuns.map((run) => ({
          ...
          items: Array.isArray(run.items) ? run.items.map((item) => ({
            ...
            nodeRuns: Array.isArray(item.nodeRuns) ? item.nodeRuns.map((node) => ({ ...node })) : undefined,
          })) : [],
          approvals: Array.isArray(run.approvals) ? run.approvals.map((entry) => ({ ...entry })) : run.approvals,
          validationReports: Array.isArray(run.validationReports) ? structuredClone(run.validationReports) : run.validationReports,
        }))
      : [],
