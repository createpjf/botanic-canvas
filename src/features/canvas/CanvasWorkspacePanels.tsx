...
const generationPanelCopy = {
  'zh-CN': {
    ...
    recovering: '正在确认任务，请勿重复提交', uploading: '正在上传画布参考素材', queued: '生成服务已接收任务，正在排队', running: '生成服务正在处理', candidatesAria: '生成结果', needsAttention: '生成需要处理', candidateTitle: (refinement: boolean, count: string | number) => `${refinement ? '精修' : '首图'} · ${count}`, refinementHint: '结果会继承父版本配方；选择后写入同一条“素材/文本 → 生成 → 结果”图谱，并可在历史中一键回退。', selectedReferencesHint: (names: string) => `真实任务以已选参考「${names}」为依据，并固定主商品。`, selectKeyVisualHint: '选择一张首图会写入结果节点、生成版本分支，并进入素材库的“生成入库”。', recipe: '生成配方', primaryName: (name: string) => `主商品 · ${name}`, inheritParent: '继承父版本', referenceCount: (count: number) => `${count} 个参考`, generatingAria: (count: number) => `正在生成 ${count} 张`, targetCount: (count: number) => `目标 ${count} 张`, cancelGeneration: '取消生成', errorFallback: '生成失败，请重试。', retry: '重试', partial: (ready: number, missing: number) => `已有 ${ready} 张可用，缺少 ${missing} 张。`, fillMissing: (count: number) => `补生成 ${count} 张`, compare: '父版本与精修对比', parentVersion: '父版本', selectedKeyVisual: '已选首图', emptyCandidates: '先在下方输入描述并发起真实生成', refinedFrom: (name: string) => `精修自 ${name}`, unlocked: '未锁定',
  },
  en: {
