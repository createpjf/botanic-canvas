...
export function TemplatePanel({
  ...
  const startAutomation = async (workflow: ProductionWorkflow) => {
    ...
    setProductionError('')
    setBatchNotice((current) => {
      if (!(workflow.id in current)) return current
      const next = { ...current }
  ...
  const updateAutomation = async (run: ProductionWorkflowRun, action: 'pause' | 'resume' | 'cancel' | 'retry-failed') => {
    ...
    try {
      ...
      if (action === 'retry-failed') {
        const previousAttempts = new Map(run.items.map((item) => [item.id, item.attempt]))
        const retried = updated.items.filter((item) => item.attempt > (previousAttempts.get(item.id) ?? 0))
        setRetryNotice((current) => ({ ...current, [run.id]: t.retryCreated(retried) }))
      }
      await refreshProductionWorkflows()
    } catch (error) {
        ...
        <section className="template-section production-workflow-section" aria-label={t.productionWorkflow}>
          ...
          <div className="production-workflow-list">
            {productionWorkflows.map((workflow) => {
              ...
              const workflowModelId = workflow.versions.at(-1)?.definition.model
              const workflowModel = generationModels.find((model) => model.id === workflowModelId)
              const provider = workflowModel?.provider === 'openai'
                ? 'OpenAI'
                : workflowModel?.provider === 'minimax' ? 'MiniMax' : workflowModel?.provider === 'flock' ? 'FLock' : 'Provider'
              ...
              return <article className="production-workflow-card" key={workflow.id}>
                ...
                <small>{provider} · {workflowModel?.label ?? workflowModelId} · {String(workflow.versions.at(-1)?.definition.output?.aspectRatio ?? '')} · {String(workflow.versions.at(-1)?.definition.output?.resolution ?? '')}</small>
                {(() => {
                ...
                {latestRun && hasFailed ? <details className="production-batch production-run-failures">
                  <summary>{t.failureDetails} · {latestRun.items.filter((item) => item.status === 'failed').length}</summary>
                  {latestRun.items.filter((item) => item.status === 'failed').map((item) => <article key={item.id}>
                    <strong>{t.batchRow(item.index + 1)} · {item.error?.code ?? 'GENERATION_FAILED'}</strong>
                    <p>{item.error?.message ?? t.failureFallback}</p>
                    <small>{t.attemptTrace(item.attempt, item.jobId)}</small>
                  </article>)}
                </details> : null}
                {latestRun && retryNotice[latestRun.id] ? <p className="production-batch__notice" role="status">{retryNotice[latestRun.id]}</p> : null}
                <footer>
